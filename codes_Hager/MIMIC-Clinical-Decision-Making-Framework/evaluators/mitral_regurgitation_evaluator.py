...
class MitralRegurgitationEvaluator(PathologyEvaluator):
    """Evaluate the trajectory according to clinical diagnosis guidelines of mitral regurgitation."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.pathology = "mitral regurgitation"
        self.alternative_pathology_names = [
            ...
            },
            {
                ...
                "modifiers": ["prolapse", "insufficien", "regurgit", "disease"],
            },
        ]
        self.gracious_alternative_pathology_names = [
            {"location": "mitral", "modifiers": ["insufficien"]},
            {"location": "valvular", "modifiers": ["disease"]},
            {"location": "mr", "modifiers": ["severe"]},
        ]
    ...
    def score_imaging(
        ...
        if region == "Chest":
            # Echocardiogram is definitive for mitral regurgitation severity
            if modality == "Echocardiogram":
                if self.scores["Imaging"] == 0:
                    self.scores["Imaging"] = 2
                return True
            # ECG — atrial fibrillation, LV strain pattern
            if modality == "ECG":
                if self.scores["Imaging"] == 0:
                    self.scores["Imaging"] = 1
                return True
            # CXR can show cardiomegaly, pulmonary congestion
    ...
    def score_treatment(self) -> None:
        ### VALVE INTERVENTION ###
        if (
            procedure_checker(MITRAL_VALVE_PROCEDURES_ICD9, self.procedures_icd9)
            or procedure_checker(
                MITRAL_VALVE_PROCEDURES_ICD10, self.procedures_icd10
            )
            or procedure_checker(
                MITRAL_VALVE_PROCEDURES_KEYWORDS, self.procedures_discharge
            )
        ):
        ...
        if (
            procedure_checker(
                MITRAL_VALVE_PROCEDURES_KEYWORDS, [self.answers["Treatment"]]
            )
            or treatment_alternative_procedure_checker(
                ALTERNATE_MITRAL_VALVE_KEYWORDS, self.answers["Treatment"]
            )
        ):
        ...
        if (
            ...
            or keyword_positive(self.answers["Treatment"], "beta blocker")
            or keyword_positive(self.answers["Treatment"], "anticoagul")
            or keyword_positive(self.answers["Treatment"], "vasodilator")
