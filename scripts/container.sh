...
# ============================================================
# container.sh — Run experiments inside Apptainer
#
# Requires vLLM to be running already (start with start_vllm.sh).
#
# Usage:
#   bash scripts/container.sh evotest 10                   # EvoTest 10 episodes
#   bash scripts/container.sh evotest --resume 15          # resume EvoTest
#   bash scripts/container.sh evotest-sdk 10               # EvoTest SDK
#   bash scripts/container.sh shell                        # interactive shell
# ============================================================
...
case "$EXPERIMENT" in
    evotest)
        EXP_CMD="bash /workspace/scripts/evotest_train.sh $*"
        DESC="EvoTest Hager ($*)"
        ;;
    evotest-sdk)
        EPISODES="${1:-10}"
    ...
    *)
        ...
        echo "Usage: $0 {evotest|evotest-sdk|shell} [args...]"
        exit 1
        ;;
esac
